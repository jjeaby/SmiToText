...
if __name__ == '__main__':
    ...
    koSpaceCheck = koSpaceCheck()

    #### 학습
    koSpaceCheck.raw2corpus(rootDirPath + '/data/koDetokenizerData/koDetokenize_Train.txt', rootDirPath + '/data/koDetokenizerData/koDetokenize_Train.txt.copus')
    koSpaceCheck.raw2corpus(rootDirPath + '/data/koDetokenizerData/rawdata/friends_smi.txt', rootDirPath + '/data/koDetokenizerData/koDetokenize_Train.txt.copus')
    koSpaceCheck.raw2corpus(rootDirPath + '/data/koDetokenizerData/koDetokenize_Valid.txt', rootDirPath + '/data/koDetokenizerData/koDetokenize_Valid.txt.copus')
    # raw_train.txt에 뭔가 긴 글이 있음


    train_sents = koSpaceCheck.corpus2sent(rootDirPath + '/data/koDetokenizerData/koDetokenize_Train.txt.copus')
    test_sents = koSpaceCheck.corpus2sent(rootDirPath + '/data/koDetokenizerData/koDetokenize_Valid.txt.copus')
    ...
    for x, y in zip(train_x, train_y):  # 파이썬2에서 돌렸다
        trainer.append(x, y)
    trainer.train(rootDirPath + '/data/koDetokenizerData/space.crfsuite')


    ## 모델 로드
    tagger = pycrfsuite.Tagger()
    tagger.open(rootDirPath + '/data/koDetokenizerData/space.crfsuite')
